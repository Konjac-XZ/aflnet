...
#
# This pauses all live fuzzers in the given output directory using
# SIGSTOP

echo "afl-pause - pause fuzzers"
echo
...
for i in `find . -maxdepth 2 -iname fuzzer_stats`; do
  ...
  if ! kill -STOP "$fuzzer_pid" 2>/dev/null; then
    echo "Unable to pause fuzzer pid $fuzzer_pid, aborting."
    echo
    exit 1
  fi
